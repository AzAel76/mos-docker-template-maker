...
var { useId: useId$1 } = await importShared("vue");
`${Object.keys({
	ł: "l",
	ø: "o",
	đ: "d",
...
var { camelize, capitalize, Comment, Fragment, isProxy, isReactive, isRef, isVNode, reactive: reactive$4, shallowRef: shallowRef$6, toRaw: toRaw$1, toRef: toRef$8, unref: unref$1, watchEffect: watchEffect$3 } = await importShared("vue");
...
function parseColor(color) {
	if (/* @__PURE__ */ isNumber(color)) {
	...
	} else if (/* @__PURE__ */ isString(color) && cssColorRe.test(color)) {
	...
	} else if (/* @__PURE__ */ isString(color)) {
		...
		return HexToRGB(hex);
	} else if (/* @__PURE__ */ isObject(color)) {
		if (has(color, [
			"r",
			"g",
			"b"
	...
	throw new TypeError(`Invalid color: ${/* @__PURE__ */ isNullOrUndefined(color) ? color : String(color) || color.constructor.name}\nExpected #hex, #hexa, rgb(), rgba(), hsl(), hsla(), object or number`);
}
...
function RGBtoHex({ r, g, b, a }) {
	return `#${[
		...
		toHex(g),
		toHex(b),
		/* @__PURE__ */ isNumber(a) ? toHex(Math.round(a * 255)) : ""
	].join("")}`;
}
function HexToRGB(hex) {
	hex = parseHex(hex);
	let [r, g, b, a] = chunk(hex, 2).map((c) => parseInt(c, 16));
	a = /* @__PURE__ */ isNumber(a) ? a / 255 : a;
	return {
		r,
		g,
		b,
...
var { computed: computed$9, inject: inject$9, provide: provide$3, ref: ref$5, shallowRef: shallowRef$5, toRef: toRef$7, unref, watchEffect: watchEffect$2 } = await importShared("vue");
...
function injectDefaults() {
	const defaults = inject$9(DefaultsSymbol);
...
function internalUseDefaults(props = {}, name, defaults = injectDefaults()) {
	...
	const componentDefaults = computed$9(() => defaults.value?.[props._as ?? name]);
	...
	function provideSubDefaults() {
		...
		provide$3(DefaultsSymbol, computed$9(() => {
...
var { computed: computed$8, shallowRef: shallowRef$4, toValue: toValue$2, watch: watch$7 } = await importShared("vue");
var easingPatterns = {
	linear: (t) => t,
	easeInQuad: (t) => t ** 2,
	easeOutQuad: (t) => t * (2 - t),
...
var { computed: computed$7, inject: inject$8, toValue: toValue$1, createVNode: _createVNode, createElementVNode: _createElementVNode, mergeProps: _mergeProps, normalizeClass: _normalizeClass } = await importShared("vue");
var IconValue = [
	String,
	Function,
	Object,
...
var { effectScope: effectScope$1, onScopeDispose: onScopeDispose$4, watch: watch$6 } = await importShared("vue");
...
var { computed: computed$6, ref: ref$4, toRaw, watch: watch$5 } = await importShared("vue");
function useProxiedModel(props, prop, defaultValue, transformIn = (v) => v, transformOut = (v) => v) {
	const vm = getCurrentInstance("useProxiedModel");
	const internal = ref$4(!/* @__PURE__ */ isUndefined(props[prop]) ? props[prop] : defaultValue);
	const kebabProp = toKebabCase(prop);
	const isControlled = kebabProp !== prop ? computed$6(() => {
		props[prop];
...
var { ref: ref$3, shallowRef: shallowRef$3, toRef: toRef$6, watch: watch$4 } = await importShared("vue");
...
function createProvideFunction(state) {
	return (props) => {
		...
		return {
			...
			fallback,
			messages,
			decimalSeparator: toRef$6(() => inferDecimalSeparator(current, fallback)),
			numericGroupSeparator: toRef$6(() => inferNumericGroupSeparator(current, fallback)),
...
function createVuetifyAdapter(options) {
	...
	return {
		...
		fallback,
		messages,
		decimalSeparator: toRef$6(() => options?.decimalSeparator ?? inferDecimalSeparator(current, fallback)),
		numericGroupSeparator: toRef$6(() => options?.numericGroupSeparator ?? inferNumericGroupSeparator(current, fallback)),
		t: createTranslateFunction(current, fallback, messages),
		n: createNumberFunction(current, fallback),
		provide: createProvideFunction({
			current,
...
var { computed: computed$5, inject: inject$7, provide: provide$2, ref: ref$2, toRef: toRef$5 } = await importShared("vue");
...
function createRtl(i18n, options) {
	...
	const isRtl = computed$5(() => rtl.value[i18n.current.value] ?? false);
	return {
		isRtl,
		rtl,
		rtlClasses: toRef$5(() => `v-locale--is-${isRtl.value ? "rtl" : "ltr"}`)
	};
}
function useRtl() {
	const locale = inject$7(LocaleSymbol);
...
var { inject: inject$6, reactive: reactive$3, watch: watch$3 } = await importShared("vue");
...
function createInstance(options, locale) {
	const instance = reactive$3(/* @__PURE__ */ isFunction(options.adapter) ? new options.adapter({
		locale: options.locale[locale.current.value] ?? locale.current.value,
		formats: options.formats
	}) : options.adapter);
	watch$3(locale.current, (value) => {
...
var { computed: computed$4, inject: inject$5, onScopeDispose: onScopeDispose$3, reactive: reactive$2, shallowRef: shallowRef$2, toRef: toRef$4, toRefs, watchEffect: watchEffect$1 } = await importShared("vue");
...
function getClientWidth(ssr) {
	return IN_BROWSER && !ssr ? window.innerWidth : /* @__PURE__ */ isObject(ssr) && ssr.clientWidth || 0;
}
function getClientHeight(ssr) {
	return IN_BROWSER && !ssr ? window.innerHeight : /* @__PURE__ */ isObject(ssr) && ssr.clientHeight || 0;
}
...
function createDisplay(options, ssr) {
	...
	watchEffect$1(() => {
		...
		const breakpointValue = /* @__PURE__ */ isNumber(mobileBreakpoint) ? mobileBreakpoint : thresholds[mobileBreakpoint];
...
function useDisplay(props = { mobile: null }, name = getCurrentInstanceName()) {
	...
	const mobile = computed$4(() => {
		if (props.mobile) return true;
		else if (/* @__PURE__ */ isNumber(props.mobileBreakpoint)) return display.width.value < props.mobileBreakpoint;
		else if (props.mobileBreakpoint) return display.width.value < display.thresholds.value[props.mobileBreakpoint];
		else if (/* @__PURE__ */ isNull(props.mobile)) return display.mobile.value;
		else return false;
	});
	const displayClasses = toRef$4(() => {
		if (!name) return {};
...
var { inject: inject$4, toRef: toRef$3 } = await importShared("vue");
...
function getTarget(el) {
	return /* @__PURE__ */ isString(el) ? document.querySelector(el) : refElement(el);
}
function getOffset(target, horizontal, rtl) {
	if (/* @__PURE__ */ isNumber(target)) return horizontal && rtl ? -target : target;
...
async function scrollTo(_target, _options, horizontal, goTo) {
	...
	const target = (/* @__PURE__ */ isNumber(_target) ? _target : getTarget(_target)) ?? 0;
	const container = options.container === "parent" && target instanceof HTMLElement ? target.parentElement : getContainer(options.container);
	const ease = PREFERS_REDUCED_MOTION() ? options.patterns.instant : /* @__PURE__ */ isFunction(options.easing) ? options.easing : options.patterns[options.easing];
	if (!ease) throw new TypeError(`Easing function "${options.easing}" not found.`);
	let targetLocation;
	if (/* @__PURE__ */ isNumber(target)) targetLocation = getOffset(target, horizontal, rtl);
	else {
...
function useGoTo(_options = {}) {
	...
	const goTo = {
		...goToInstance,
		rtl: toRef$3(() => goToInstance.rtl.value || isRtl.value)
	};
...
var { computed: computed$3, getCurrentScope, inject: inject$3, onScopeDispose: onScopeDispose$2, provide: provide$1, ref: ref$1, shallowRef: shallowRef$1, toRef: toRef$2, watch: watch$2, watchEffect } = await importShared("vue");
...
function genCssVariables(theme, prefix) {
	...
	for (const [key, value] of Object.entries(theme.colors)) {
		...
		variables.push(`--${prefix}theme-${key}: ${rgb.r},${rgb.g},${rgb.b}` + (/* @__PURE__ */ isNumber(rgb.a) ? `,${rgb.a}` : ""));
	...
	for (const [key, value] of Object.entries(theme.variables)) {
		const color = /* @__PURE__ */ isString(value) && value.startsWith("#") ? parseColor(value) : void 0;
		const rgb = color ? `${color.r}, ${color.g}, ${color.b}` + (/* @__PURE__ */ isNumber(color.a) ? `, ${color.a}` : "") : void 0;
...
function genVariation(name, color, variations) {
	...
	if (variations) for (const variation of ["lighten", "darken"]) {
		...
		for (const amount of /* @__PURE__ */ range(variations[variation], 1)) object[`${name}-${variation}-${amount}`] = RGBtoHex(fn(parseColor(color), amount));
	}
	return object;
}
...
function createTheme(options) {
	...
	if (SUPPORTS_MATCH_MEDIA) {
		...
		if (getCurrentScope()) onScopeDispose$2(() => {
	...
	function install(app) {
		...
		if (head) {
			...
			if (IN_BROWSER) watch$2(styles, updateStyles, { immediate: true });
			else updateStyles();
	...
	function resolveTransitionOptions(transition) {
		...
		const global = /* @__PURE__ */ isObject(parsedOptions.transition) ? parsedOptions.transition : {};
		const local = /* @__PURE__ */ isObject(transition) ? transition : {};
		return {
...
var { computed: computed$2, inject: inject$2, onActivated, onBeforeUnmount, onDeactivated, onMounted, onScopeDispose: onScopeDispose$1, provide, reactive: reactive$1, ref, shallowRef, toRef: toRef$1, useId, watch: watch$1 } = await importShared("vue");
...
function parseKeyCombination(input) {
	...
	function parseKey() {
		const ch = peek();
		if (!/* @__PURE__ */ isString(ch)) throw new ParseError("Unexpected end of input");
		const next = peek(1);
		if (isSep(ch) && /* @__PURE__ */ isString(next) && !isSep(next)) throw new ParseError(`Unexpected separator '${ch}' at position ${pos}`);
		const first = consume();
...
var { onScopeDispose, toValue, watch } = await importShared("vue");
var MODIFIERS = [
	"ctrl",
	"shift",
	"alt",
...
function useHotkey(keys, callback, options = {}) {
	...
	watch(() => toValue(keys), (newKeys) => {
		...
		if (newKeys) {
			...
			if (parsed) {
				const parts = !/* @__PURE__ */ isString(parsed) && parsed.type === "sequence" ? parsed.parts : [parsed];
...
function matchesKeyGroup(e, group, isMac) {
	if (!/* @__PURE__ */ isString(group) && group.type === "alternate") return group.parts.some((part) => matchesKeyGroup(e, part, isMac));
...
function parseKeyGroup(group) {
	const parts = /* @__PURE__ */ isString(group) ? [group] : group.parts;
...
var { computed: computed$1 } = await importShared("vue");
...
function useMask(props) {
	const mask = computed$1(() => {
		if (/* @__PURE__ */ isString(props.mask)) {
			if (props.mask in presets) return presets[props.mask];
			return props.mask;
		}
		return props.mask?.mask ?? "";
	});
	const tokens = computed$1(() => {
		return {
			...defaultTokens,
			.../* @__PURE__ */ isObject(props.mask) ? props.mask.tokens : null
		};
	});
	...
	function maskValidates(mask, char) {
		if (!/* @__PURE__ */ isString(char) || !isMask(mask)) return false;
	...
	function maskText(text) {
		...
		if (!/* @__PURE__ */ isString(trimmedText)) return "";
	...
	function unmaskText(text) {
		if (!/* @__PURE__ */ isString(text)) return null;
	...
	function getUnmaskMap(text) {
		if (!/* @__PURE__ */ isString(text) || !mask.value.length || !text.length) return [];
		let textIndex = 0;
		let maskIndex = 0;
		...
		while (true) {
			...
			if (!/* @__PURE__ */ isString(tchar)) break;
			if (!/* @__PURE__ */ isString(mchar)) {
			...
			if (maskValidates(mchar, tchar)) {
				...
				while (true) {
					const mchar = mask.value[maskIndex++];
					if (!/* @__PURE__ */ isString(mchar) || maskValidates(mchar, tchar)) break;
				}
				continue;
			}
			textIndex++;
...
var { computed, inject: inject$1, toRef } = await importShared("vue");
function createRules(options, locale) {
	...
	const aliases = {
		...
		email: (err) => {
			return (v) => !v || /* @__PURE__ */ isString(v) && /^.+@\S+\.\S+$/.test(v) || t(err || "$vuetify.rules.email");
		},
		number: (err) => {
	...
	function resolve(fn) {
		return computed(() => fn().map((rule) => {
			...
			} else if (/* @__PURE__ */ isString(rule)) ruleName = rule;
			if (ruleName) {
...
var { effectScope, nextTick, reactive } = await importShared("vue");
